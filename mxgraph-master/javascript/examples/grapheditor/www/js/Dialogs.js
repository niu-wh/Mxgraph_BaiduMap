...
var FilenameMapDialog = function(editorUi, filename, buttonText, fn, label, validateFn, content, helpLink, closeOnBtn, cancelFn)
{
	closeOnBtn = (closeOnBtn != null) ? closeOnBtn : true;
	var row, td;
	...
	if (helpLink != null)
	{
		var helpBtn = mxUtils.button(mxResources.get('map'), function()
		{
			editorUi.editor.graph.openLink(helpLink);
		});
...
var EditDataDialog = function(ui, cell)
{
	...
	if (ui.editor.graph.getModel().isVertex(cell) || ui.editor.graph.getModel().isEdge(cell))
	{
		...
		if (EditDataDialog.placeholderMapLink != null)
		{
			var link = document.createElement('a');
			link.setAttribute('href', EditDataDialog.placeholderMapLink);
			link.setAttribute('title', mxResources.get('map'));
			link.setAttribute('target', '_blank');
			link.style.marginLeft = '10px';
			link.style.cursor = 'map';
...
EditDataDialog.placeholderMapLink = null;
